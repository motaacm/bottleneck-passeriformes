...
`pre.txt` and `pos.txt` are the FID and FII columns respectively with the data present in `het_allpresent.het` for each pre- and post-accident sample.
...
### ON ENBIRONMENT (R) – Mann-Whitney test to compare the heterozigosity difference found in the groups
...
### ON ENVIRONMENT – Summarizing (mean/median) and number of windows
...
### Calculating FROH via console (R)
...
### Comparison between pre and post + graph (R)
...
### 3) Generating graph and statistical test (R)
